...
object Float64Math {
     ...
     */
    fun isZero(bits: Long): Boolean = (bits and 0x7FFF_FFFF_FFFF_FFFFL) == 0L

    /**
     * Get sign of float64 value.
     * @return 1 for negative, 0 for positive
     */
    fun getSign(bits: Long): Int = ((bits and SIGN_MASK) ushr 63).toInt()

    /**
     * Negate float64 value (flip sign bit).
     */
    fun negateBits(bits: Long): Long = bits xor SIGN_MASK

    /**
     * Absolute value (clear sign bit).
     */
    fun absBits(bits: Long): Long = bits and 0x7FFF_FFFF_FFFF_FFFFL
